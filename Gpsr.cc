...
//void Gpsr::broadcastMessage(cMessage *message, const L3Address& destination, const L3Address& source)
//{
...
void Gpsr::sendBeacon(const Ptr<GpsrBeacon>& beacon)
{
    ...
    Packet *udpPacket1 = new Packet("BroadcastBeacon");
...
//        case GPSR_GREEDY_FORWARDING: return greedyForwarding(destination, gprsOption);
...
//    This is step 1, the forwarder upon receiving the packet, calculates the distance between itself
//    and destination as bestDistance
    ...
    std::vector<L3Address> neighborAddresses = neighborPositionTable.getAddresses();
//    This is step 2, for all its neighbors, the forwarder calculates the distance between the neighbor
//    and the destination
    for (auto& neighborAddress: neighborAddresses) {
        Coord neighborPosition = neighborPositionTable.getPosition(neighborAddress);
        double neighborDistance = (destinationPosition - neighborPosition).length();
        if (neighborDistance < bestDistance) { // This is step 3, if a smaller distance is found, then choose the best option
            bestDistance = neighborDistance;
            bestNeighbor = neighborAddress;
        }
    }
    // TODO: modification needed where the forwarder either pushes the message in the buffer for later use when a node closer to
    // the destination is found.
    // TODO: modification needed where the forwarder broadcasts the message aka starts CBF algorithm
    if (bestNeighbor.isUnspecified()) {

        //        This is the edge case where a forwarder is unable to find a neighbor and does one of the following:
        //        1. Buffers the packet so when a node enters the range of the forwarder, which is a valid node, i.e. closer to the destination compared to
        //        the forwarder, the forwarder forwards the packet to the node.
        //        2. Broadcasts the packet to any node in the range of the forwarder.
    }
    else
        return bestNeighbor;
}

L3Address Gpsr::greedyForwarding(const L3Address& destination, GpsrOption *gpsrOption)
{
    EV_DEBUG << "Finding next hop using greedy routing: destination = " << destination << endl;
    L3Address selfAddress = getSelfAddress();
    Coord selfPosition = mobility->getCurrentPosition();
